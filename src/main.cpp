...
#if defined(NDEBUG)
# error "Suncoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x88a58e9ac8a783b93484e3cb8a07a829379b1b84541ec6f788a0064497907145");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Suncoin: starting difficulty is 1 / 2^12
...
{
    // Suncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Suncoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60 * 60; // Suncoin: 1 hour
static const int64 nTargetSpacing =  6 * 60; // Suncoin: 6 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Suncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Suncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x88a58e9ac8a783b93484e3cb8a07a829379b1b84541ec6f788a0064497907145");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is Suncoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1493465355;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 4600411;
        ...
        assert(block.hashMerkleRoot == uint256("0xb9fb84bd617c239e0c83b4a23681ae97708217dbf9e63052cf411c206b5feeda"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Suncoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SuncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SuncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SuncoinMiner(CWallet *pwallet)
{
    printf("SuncoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running SuncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SuncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SuncoinMiner, pwallet));
}
